...
df=df.fillna("")
familypos= df['Department: '] + df ['Position Title:']
...
#Tsne = TSNE(n_componenfamilypos=2, verbose=2, perplexity=30, n_iter=100, random_state=15, learning_rate=2000, early_exaggeration=100)
#Tsne_results = familyposne.fit_transform(familypos_data)



sse = {}
for k in range(1, 45):
    kmeans = KMeans(n_clusters=k, max_iter=1000).fit(familypos_vec)
    data = kmeans.labels_
    #print(data["clusters"])
...
for n_cluster in range(2, 40,2):
    kmeans = KMeans(n_clusters=n_cluster).fit(familypos_vec)
